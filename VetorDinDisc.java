 ...
 */
public class VetorDinDisc implements IArmazenadorD, java.io.Serializable
{
    // Atributos
    ...
    public boolean adicionar(Object obj){
        if(estaVazia() == false){
            for(int i = 0; i < getQtd(); i++){
                if(array[i] instanceof Disciplina && ((Disciplina) array[i]).codigo.equals(((Disciplina) obj).codigo) || array[i] instanceof Disciplina && ((Disciplina) array[i]).nomeDisc.equals(((Disciplina) obj).nomeDisc))
                    return false;
            }
        }
    ...
    public boolean remover(String codigo){
        if(estaVazia() == false){
            for(int i = 0; i < getQtd(); i++){
                if(array[i] instanceof Disciplina && ((Disciplina) array[i]).codigo.equals(codigo)){
                    array[i] = null;
                    if(getQtd() > 1){
